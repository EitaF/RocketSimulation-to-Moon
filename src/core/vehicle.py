...
class Rocket:
    ...
    def get_thrust_vector(self, t: float, altitude: float = 0.0) -> Vector3:
        ...
        # Check if stage is actively burning
        ...
        # For now, return vertical thrust (will be modified by guidance)
        # In a full implementation, this would consider vehicle attitude
    ...
    def separate_stage(self, current_time: float) -> bool:
        ...
        if self.current_stage < len(self.stages) - 1:
            ...
            if hasattr(self, 'current_stage_burn_time'):
                self.current_stage_burn_time = 0.0
            return True
        return False
    ...
    def update_stage(self, dt: float):
        ...
        """
        if not hasattr(self, 'current_stage_burn_time'):
            self.current_stage_burn_time = 0.0
        
        # Update burn time for current stage
        stage = self.current_stage_obj
        if stage and self.is_thrusting(self.current_stage_burn_time + self.stage_start_time, 0):
            self.current_stage_burn_time += dt
            
            # Check if stage should separate (burn time exceeded)
            if self.current_stage_burn_time >= stage.burn_time:
                if self.current_stage < len(self.stages) - 1:
                    # Auto-separate to next stage
                    self.separate_stage(self.current_stage_burn_time + self.stage_start_time)
                    self.current_stage_burn_time = 0.0
